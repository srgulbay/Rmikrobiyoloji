...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    const payload = {
      ...
      specialization: user.specialization, defaultClassificationId: user.defaultClassificationId
    };
    ...
    res.status(200).json({
      message: 'Giriş başarılı.', token: token,
      user: {
        id: user.id, username: user.username, role: user.role,
        specialization: user.specialization, isEmailVerified: user.isEmailVerified,
...
// YENİ FONKSİYON: Giriş yapmış kullanıcının şifresini değiştirmesi
const changePassword = async (req, res) => {
  const userId = req.user.id; // protect middleware'inden gelir
  const { currentPassword, newPassword, confirmNewPassword } = req.body;

  if (!currentPassword || !newPassword || !confirmNewPassword) {
    return res.status(400).json({ message: 'Lütfen tüm şifre alanlarını doldurun.' });
  }
  ...
  try {
    ...
    if (!user) {
      // Bu durum normalde protect middleware'i nedeniyle oluşmamalı
...
module.exports = {
  ...
  resetPassword,
  changePassword // YENİ fonksiyon export edildi
};
